@php
    ...
    $navItems = [
        ...
        [
            ...
            // One entry per quote type — each opens a fresh form of that type.
            'children' => collect(\App\Models\QuoteTemplate::types())
                ->map(fn ($cfg, $key) => [
                    'route'  => 'quote-templates.create',
                    'params' => ['type' => $key],
                    'label'  => $cfg['label'],
                ])->values()->all(),
        ],
        [
            'route' => 'blog-posts.index', 'active' => 'blog-*', 'label' => 'Blog',
            ...
            'children' => [
                ['route' => 'blog-posts.create',     'params' => [], 'label' => 'Artikel Baru'],
                ['route' => 'blog-categories.index', 'params' => [], 'label' => 'Kategori'],
            ],
        ],
        [
            'route' => 'contact.index', 'active' => 'contact.*', 'label' => 'Mesej Hubungi',
    ...
    // A child is active only when the current request is on its exact route AND
    // every one of its params matches (so ?type=X highlights only that one).
    $childActive = fn ($child) => request()->routeIs($child['route'])
        && collect($child['params'] ?? [])->every(fn ($v, $k) => (string) request()->query($k) === (string) $v);

    // The parent's own "Semua" link is active on the index with no child param.
    $indexActive = fn ($item) => request()->routeIs($item['route'])
        && ! request()->filled('site') && ! request()->filled('type');

    $childLink = 'block rounded-md px-2 py-1.5 text-sm text-orange-100 hover:bg-orange-700 hover:text-white';
@endphp
<!DOCTYPE html>
                                    ...
                                    <li>
                                        <ul role="list" class="-mx-2 space-y-1">
                                            @foreach ($navItems as $item)
                                                <li @if(! empty($item['children'])) x-data="{ open: {{ request()->routeIs($item['active']) ? 'true' : 'false' }} }" @endif>
                                                    @if(! empty($item['children']))
                                                        <button type="button" @click="open = ! open"
                                                                class="{{ $linkClasses }} w-full {{ request()->routeIs($item['active']) ? 'bg-orange-700 text-white' : '' }}">
                                                            <svg class="size-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">{!! $item['icon'] !!}</svg>
                                                            <span class="flex-1 text-left">{{ $item['label'] }}</span>
                                                            <svg class="size-4 shrink-0 transition-transform" :class="open && 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                                                        </button>
                                                        <ul role="list" x-show="open" x-cloak class="mt-1 space-y-1 pl-9">
                                                            <li>
                                                                <a href="{{ route($item['route']) }}" @click="closeSidebar"
                                                                   class="{{ $childLink }} {{ $indexActive($item) ? 'bg-orange-700 text-white font-semibold' : '' }}">Semua</a>
                                                            </li>
                                                            @foreach ($item['children'] as $child)
                                                                <li>
                                                                    <a href="{{ route($child['route'], $child['params'] ?? []) }}" @click="closeSidebar"
                                                                       class="{{ $childLink }} {{ $childActive($child) ? 'bg-orange-700 text-white font-semibold' : '' }}">
                                                                        {{ $child['label'] }}
                                                                    </a>
                                                                </li>
                                                            @endforeach
                            ...
                            <li>
                                <ul role="list" class="-mx-2 space-y-1">
                                    @foreach ($navItems as $item)
                                        <li @if(! empty($item['children'])) x-data="{ open: {{ request()->routeIs($item['active']) ? 'true' : 'false' }} }" @endif>
                                            @if(! empty($item['children']))
                                                <button type="button" @click="isSidebarCollapsed ? (isSidebarCollapsed = false, open = true) : (open = ! open)"
                                                        :title="isSidebarCollapsed ? @js($item['label']) : ''"
                                                        class="{{ $linkClasses }} w-full {{ request()->routeIs($item['active']) ? 'bg-orange-700 text-white' : '' }}"
                                                        :class="isSidebarCollapsed ? 'justify-center' : ''">
                                                    <svg class="size-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">{!! $item['icon'] !!}</svg>
                                                    <span x-show="!isSidebarCollapsed" class="flex-1 text-left whitespace-nowrap">{{ $item['label'] }}</span>
                                                    <svg x-show="!isSidebarCollapsed" class="size-4 shrink-0 transition-transform" :class="open && 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                                                </button>
                                                <ul role="list" x-show="open && !isSidebarCollapsed" x-cloak class="mt-1 space-y-1 pl-9">
                                                    <li>
                                                        <a href="{{ route($item['route']) }}"
                                                           class="{{ $childLink }} whitespace-nowrap {{ $indexActive($item) ? 'bg-orange-700 text-white font-semibold' : '' }}">Semua</a>
                                                    </li>
                                                    @foreach ($item['children'] as $child)
                                                        <li>
                                                            <a href="{{ route($child['route'], $child['params'] ?? []) }}"
                                                               class="{{ $childLink }} whitespace-nowrap {{ $childActive($child) ? 'bg-orange-700 text-white font-semibold' : '' }}">
                                                                {{ $child['label'] }}
                                                            </a>
                                                        </li>
                                                    @endforeach
